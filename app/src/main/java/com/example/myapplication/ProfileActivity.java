...
public class ProfileActivity extends AppCompatActivity {
    ...
    private EditText etName, etEmail, etDob, etPassword;
    TextView txtBack;
    RadioGroup rgGender;
    RadioButton rbMale;
    RadioButton rbFemale;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // ---- Lấy userId và Role từ session ----
        int userId = sharedPref.getInt("user_id", -1);
        final String[] userRole = {"user"}; // Sử dụng final array để có thể truy cập từ inner class

        if (userId != -1) {
            Cursor cursor = databaseHelper.getUserById(userId);
            if (cursor != null && cursor.moveToFirst()) {
                @SuppressLint("Range") String username = cursor.getString(cursor.getColumnIndexOrThrow(DatabaseHelper.COLUMN_USER_USERNAME));
                @SuppressLint("Range") String email = cursor.getString(cursor.getColumnIndexOrThrow(DatabaseHelper.COLUMN_USER_EMAIL));
                @SuppressLint("Range") String gender = cursor.getString(cursor.getColumnIndexOrThrow(DatabaseHelper.COLUMN_USER_GENDER));
                @SuppressLint("Range") String dob = cursor.getString(cursor.getColumnIndexOrThrow(DatabaseHelper.COLUMN_USER_DOB));
                @SuppressLint("Range") String password = cursor.getString(cursor.getColumnIndexOrThrow(DatabaseHelper.COLUMN_USER_PASSWORD));
                @SuppressLint("Range") String role = cursor.getString(cursor.getColumnIndexOrThrow(DatabaseHelper.COLUMN_USER_ROLE));
                userRole[0] = role; // Gán giá trị role
        ...
        btnUpdate.setOnClickListener(v -> {
            ...
            int rowsAffected = databaseHelper.updateUser(userId, newName, newEmail, newPassword, newGender, newDob, userRole[0]);
